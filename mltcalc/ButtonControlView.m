...
{
    [super viewDidLoad];
    text_view_rect=CGRectMake(2, 53, 317, 155);
...
//#endif
    ...
    // PaymentManagerが発行する購入処理が失敗した時の通知を受け取るように登録する
...
-(void)selectMenu:(NSString*)arg_str {
    ...
    if (select_help_view!=nil) {
        ...
        NSString *FileMenu_NoPayment=NSLocalizedString(@"HelpMenu_NoPayment", @"広告を停止する");
        NSString *HelpMenu_Restore=@"Restore";
        if ([arg_str isEqualToString:HelpMenu_video]) {
            [self rightButtonAction];
        }else if ([arg_str isEqualToString:FileMenu_NoPayment]) {
            [self stopIad];
        }else if ([arg_str isEqualToString:HelpMenu_Restore]) {
        }
    }
    if (select_view!=nil) {
        [select_view removeFromSuperview];
